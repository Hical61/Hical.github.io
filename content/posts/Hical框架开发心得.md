+++
title = 'Hical 框架开发心得：七个深刻教训'
date = '2026-05-18'
draft = false
tags = ["C++20", "C++26", "框架设计", "开发心得", "Hical", "WebSocket"]
categories = ["Hical框架"]
description = "分享 Hical 框架开发至 v2.6.2 过程中的真实体会：协程陷阱、PMR 内存池、去 Beast 重写、WebSocket 广播架构，以及取舍背后的逻辑。"
+++
...
Hical 是一个基于 Boost.Asio 的现代 C++20/26 高性能 Web 框架，采用自研 HTTP/WebSocket 栈（picohttpparser + 手写 RFC 6455 实现），从第一行代码到现在的 45+ 测试文件、3 层内存池、协程化数据库中间件、自研日志系统、OpenAPI 自动生成、WsHub 广播管理器、QPS 从 27K 到 159K 的优化历程，一路走来踩了不少坑，也收获了很多。
...
- [Hical 框架开发心得：七个深刻教训](#hical-框架开发心得七个深刻教训)
  - [引言](#引言)
  - [目录](#目录)
  ...
  - [七、WsHub 广播管理器——从"能用"到"好用"的 WebSocket 架构](#七wshub-广播管理器从能用到好用的-websocket-架构)
    - [7.1 问题：裸连接管理的困境](#71-问题裸连接管理的困境)
    - [7.2 WsHub 的核心设计](#72-wshub-的核心设计)
    - [7.3 写串行化：被忽视的并发陷阱](#73-写串行化被忽视的并发陷阱)
    - [7.4 收获：框架应该管理连接生命周期](#74-收获框架应该管理连接生命周期)
  - [后记：ThreadSanitizer CI 揪出的隐藏竞态（v2.6）](#后记threadsanitizer-ci-揪出的隐藏竞态v26)
    - [Boost.Asio 对象的跨线程操作](#boostasio-对象的跨线程操作)
    - [`stop()` 的并发调用：门卫模式](#stop-的并发调用门卫模式)
    - [教训](#教训)
  - [总结：七条核心原则](#总结七条核心原则)
...
v2.6.2 引入了 `WsHub` 广播管理器，核心设计原则是**读写分离 + 跨线程安全投递**：
...
广播操作的关键在于**跨线程安全写入**：每个 `WebSocketSession` 绑定在特定的 io_context 线程上（SO_REUSEPORT 多 acceptor 模型），从 Hub 线程直接调用 `send()` 是跨线程操作——违反 Asio 的线程安全契约（后记中 TSan 揪出的同类问题）。
...
1. **`weak_ptr` 存储**：Hub 不延长连接生命周期。连接断开后 `weak_ptr::lock()` 返回 nullptr，广播时自动跳过。用户必须在 `onDisconnect` 回调中调用 `remove(id)` 清理条目
2. **`shared_ptr<string>` 共享消息**：N 个目标连接共享同一份消息内存，避免 N 次字符串拷贝。但单目标 `sendTo()` 直接 move string 进 lambda，省去 `shared_ptr` 控制块分配
...
- **同一 socket 上的并发写操作需要串行化**——用 `steady_timer` 做协程信号量，比裸 mutex 更契合协程模型
- **`weak_ptr` 是管理非拥有引用的正确工具**——Hub 不应该延长连接生命周期，否则断开的连接无法被真正释放
...
7. **业务层容易写错的代码应由框架提供**：连接注册、房间分组、跨线程投递、写串行化——这些单独不复杂，但组合在一起极易出错。WsHub 的经验证明，框架多做一点，业务少踩一个坑
