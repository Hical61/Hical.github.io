...
这篇文章不追求完整列举所有提案，只聊我认为**对实际项目冲击最大**的特性——尤其从游戏服务器和 Hical 框架开发的角度。这是 [C++17 心得]({{< relref "posts/cpp/cpp17-features-in-practice.md" >}}) 和 [C++20 心得]({{< relref "posts/cpp/cpp20-features-in-practice.md" >}}) 的续篇。
...
50 个 DTO 结构体 × 10 个字段 = 500 行手写映射代码，现在全部自动化。我在 Hical 框架中已经用[双路线策略]({{< relref "posts/hical/03-cpp26-reflection-dual-track.md" >}})提前布局了这一特性。
...
#### 深入理解：constexpr vector 编译出来到底是什么？

这个特性容易让人困惑——"编译期分配的堆内存存活到运行期"听起来很魔幻，vector 编译出来到底是个啥？分几层来讲：

**C++20 的限制：编译期 `new` 必须编译期 `delete`**

C++20 允许在 `constexpr` 函数里用 `new`/`delete` 和 `std::vector`，但有一条铁律：**所有编译期分配的堆内存，必须在编译期内释放干净**。你可以在 `constexpr` 函数内部用 vector 做计算，但函数返回时这个 vector 必须已经析构了，不能把它"带出来"：

```cpp
// C++20 合法 —— vector 在函数内部用完就销毁了
constexpr int countPrimes(int limit) {
    std::vector<int> primes;
    // ... 填充 ...
    return primes.size(); // 返回 int，vector 在这里析构，内存释放
}
constexpr int n = countPrimes(100); // OK，编译期算出 25
```

```cpp
// C++20 非法 —— 试图把 vector 带到运行期
constexpr auto primes = generatePrimes(100); // 编译错误！
// 错误原因：vector 内部的堆内存在编译期分配，但没有在编译期释放
```

**C++26 放宽：编译期分配的内存可以"存活"到运行期**

C++26 的提案（P2738R1）允许编译期分配的内存跨越到运行期。那编译器具体怎么做的？

**核心机制：编译器把编译期的堆分配"降级"为静态存储。**

`constexpr auto primes = generatePrimes(10000);` 编译后大致等价于：

```cpp
// 编译器在编译期执行 generatePrimes(10000)，算出结果是 {2, 3, 5, 7, 11, ...}
// 然后生成类似这样的东西（伪代码，展示本质）：

// 1) 一块静态存储区，存放算好的数据
static constexpr int __primes_data[] = {2, 3, 5, 7, 11, 13, ..., 9973};

// 2) primes 这个变量本身，编译器让它指向这块静态数据
//    size = 1229, capacity = 某个值, data_ptr = &__primes_data[0]
```

**它不是运行时在堆上 malloc 一块内存，而是编译期算好所有元素值后，把数据直接烘焙（bake）进二进制的 `.rodata` 段（只读数据段）。**

可以理解为编译器做了一次"快照"：
- **编译期**：正常执行 vector 的 push_back、扩容等全部逻辑
- **编译完成时**：把 vector 内部堆上的最终状态，**平铺**成一块静态常量数据
- **运行期**：`primes` 变量直接引用这块已经写死在二进制里的数据，**零运行时开销**

**和 `constexpr std::array` 的区别**

你可能会问：那这和手写一个 `constexpr std::array` 有什么区别？

区别在于**编程体验**：
- `std::array` 要求编译期就知道大小，你得写 `std::array<int, 1229>`，但 1229 这个数你还得先算出来
- `std::vector` 允许你用动态逻辑（push_back、条件判断）去构建数据，编译器最后帮你"固化"成静态数据

**最终产物是一样的** —— 都是二进制里一段只读数据。但 vector 版本让你用更自然的代码去生成这段数据。

> **一句话总结**：`constexpr std::vector` 编译出来不是一个"运行时的堆对象"，而是**一段嵌入在二进制 `.rodata` 中的只读数据** + 一个编译期确定的 size/capacity 元信息。编译期的 `new` 是编译器内部的虚拟内存操作，从未真正调用过系统的 `malloc`。
...
| 阶段           | 时间点         | 行动                                                                     |
| -------------- | -------------- | ------------------------------------------------------------------------ |
| 现在           | 2026 Q2        | 用双路线策略预埋反射接口（[已实践]({{< relref "posts/hical/03-cpp26-reflection-dual-track.md" >}})） |
...
> 推荐阅读：[C++17 实战心得]({{< relref "posts/cpp/cpp17-features-in-practice.md" >}}) | [C++20 实战心得]({{< relref "posts/cpp/cpp20-features-in-practice.md" >}}) | [C++26 反射双路线实战]({{< relref "posts/hical/03-cpp26-reflection-dual-track.md" >}})
