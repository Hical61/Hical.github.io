...
> **系列导航**：[第一篇：设计理念](#)（本文） | [第二篇：协程与内存池]({{< relref "从零构建现代C++ Web服务器（二）" >}}) | [第三篇：路由、中间件与SSL]({{< relref "从零构建现代C++ Web服务器（三）" >}}) | [第四篇：实战与性能]({{< relref "从零构建现代C++ Web服务器（四）" >}}) | [第五篇：Cookie、Session与文件服务]({{< relref "从零构建现代C++ Web服务器（五）" >}})
