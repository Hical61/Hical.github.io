...
> [上一篇横评]({{< relref "posts/performance/C++Web框架横评.md" >}})我们从架构设计、功能完整度和开发体验角度对比了四个 C++ Web 框架。结论是"各有适合的场景"——但没回答一个关键问题：**到底差多少**。本文用硬数据补上这个缺口：相同硬件、相同容器、相同压测工具，12 个场景全量对比，包括别人不太敢贴的对自己不利的数据。
...
- **补充 [07 号文章]({{< relref "posts/performance/C++Web框架横评.md" >}})** 的定性对比，用数据量化各框架的性能差异
- **与 [11 号文章]({{< relref "posts/performance/cpp-vs-go-rust-web.md" >}})** 的跨语言对比形成互补——那篇回答"C++ 和 Go/Rust 差多少"，本篇回答"C++ 框架之间差多少"
