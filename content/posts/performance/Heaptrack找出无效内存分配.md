...
*上一篇：[《perf + 火焰图：5 分钟定位 C++ 程序的 CPU 瓶颈》]({{< relref "posts/performance/perf火焰图定位CPU瓶颈.md" >}})——从 CPU 层面定位热点函数。*

*下一篇：[《缓存行对 C++ 性能的影响有多大？实测告诉你》]({{< relref "posts/performance/缓存行对性能的影响.md" >}})——为什么 vector 比 map 快 10 倍？为什么两个线程写不同变量也会互相拖慢？*
