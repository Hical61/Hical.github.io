...
防范退化靠的不是文档里一段注释，是自动化检测。不过坦白说，Hical 目前在这一块还是空白——下面写的方案是我计划中要做的事，而不是已经落地的。

### 现状：GTest 性能用例

Hical 目前有一份 [test_router_perf.cpp](https://github.com/hical/Hical/blob/main/tests/test_router_perf.cpp)，用 GTest + 手动 `std::chrono` 计时覆盖了 `resolveRoute`、`dispatchSync` 的性能断言。这不是 Google Benchmark，没有 CI 回归对比——CI 甚至主动排除了 `RouterPerfTest`（`-E RouterPerfTest`），因为 CI runner 的 CPU 波动让断言不稳定。

```cpp
// test_router_perf.cpp — Hical 实际的性能测试方式
TEST_F(RouterPerfTest, DispatchSync_SyncHandler)
{
    // ... 预热 1000 次、跑 100000 次、手算 nsPerOp
    EXPECT_LT(nsPerOp, 500.0);  // dispatchSync 应在 < 500ns
}
```

### 方案一：引入 Google Benchmark 进 CI

这是下一步最值得做的事：把关键路径的微基准跑进 CI，每次 PR 对比基线。

```cpp
// tests/benchmarks/bench_router.cpp（Hical 计划中，非现状）
#include <benchmark/benchmark.h>

static void BM_RouteLookup(benchmark::State& state) 
{
    Router router;
    // 注册 1000 条路由模拟生产环境
    for (int i = 0; i < 1000; ++i)
        router.addRoute(hGet, "/api/v1/user/" + std::to_string(i), handler);
    for (auto _ : state) {
        auto result = router.resolveRoute(hGet, "/api/v1/user/500");
...
```bash
# CI 中：跑 benchmark，和上次 commit 对比
cmake --build build --target benchmarks
# 跑当前 commit 的 benchmark
./build/tests/benchmarks --benchmark_out=results_new.json --benchmark_out_format=json
# 切到基线 commit 跑同样的 benchmark
git stash && git checkout main
cmake --build build --target benchmarks
./build/tests/benchmarks --benchmark_out=results_baseline.json --benchmark_out_format=json
# Google Benchmark 自带对比工具
python3 benchmark/tools/compare.py benchmarks results_baseline.json results_new.json
```

Google Benchmark 自带 `tools/compare.py` 做统计对比，会标出有显著差异的 benchmark。设定退化阈值也可以在 CI 里做——超 5% 退化就失败。

### 方案二：wrk 冒烟测试（Hical 规划中）

CI 里拉服务跑 10s wrk，吞吐低于基线就报警：

```yaml
# GitHub Actions 片段
- name: Start server
  run: ./build/hical_test_server &
- name: Smoke test
  run: |
    QPS=$(wrk -c100 -t4 -d10s http://localhost:8080/health | grep 'Requests/sec' | awk '{print $2}')
    if (( $(echo "$QPS < 50000" | bc -l) )); then
      echo "QPS ${QPS} below threshold 50000, regression detected"
      exit 1
    fi
```

这个不能替代微基准（CI 机器的负载波动让结果不够精确），但能拦住明显的退化——比如有人不小心在主循环里加了把锁。
...
1. **QPS 的必经路径**：`resolveRoute`、`dispatchSync`、`processRequest`（前两个已有 GTest 用例，可以升级为 Google Benchmark）
