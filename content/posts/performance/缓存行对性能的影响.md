...
*上一篇：[《Heaptrack：找出 C++ 程序中的无效内存分配》]({{< relref "posts/performance/Heaptrack找出无效内存分配.md" >}})——定位和消灭无效的堆分配。*

*第一篇：[《perf + 火焰图：5 分钟定位 C++ 程序的 CPU 瓶颈》]({{< relref "posts/performance/perf火焰图定位CPU瓶颈.md" >}})——从 CPU 层面定位热点函数。*
