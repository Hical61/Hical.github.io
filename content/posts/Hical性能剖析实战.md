...
> 在 [C++ 框架性能实测](21-cpp-framework-benchmark.md)中，Hical 的 Hello World QPS（~27K）远低于 Cinatra（165K）和 Drogon（161K）。静态链接 + strip 验证后确认瓶颈不在链接方式。本文记录用 `perf record` + 火焰图精确定位 CPU 热点的全过程。
