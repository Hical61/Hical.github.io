...
> 在 [火焰图分析](22-hical-profiling-flamegraph.md)中，我们定位到 Hical 的 QPS 瓶颈在 Boost.Asio 的 epoll 交互模型——跨线程调度（14.5%）和 timer 相关 epoll_ctl（12.5%）合计吃掉了 27% 的 CPU。[P1 优化](../CHANGELOG.md)（Router 同步快速路径）无实质提升后，本文记录 P2/P3 两项优化的设计思路、实现细节和实测结果。
...
```bash
# 还原为远程仓库原版
git checkout origin/main -- src/core/HttpServer.h src/core/HttpServer.cpp src/core/HttpSessionImpl.cpp
cmake --build build -j$(nproc)
./build/bench_server &
wrk -t4 -c100 -d30s http://127.0.0.1:8080/
pkill bench_server
