...
> **课程导航**：[学习路径]({{< relref "posts/boost/Boost库学习课程_学习路径导航.md" >}}) | [Boost.System]({{< relref "posts/boost/Boost.System_错误处理基石.md" >}}) | [Boost.Asio]({{< relref "posts/boost/Boost.Asio_异步IO与协程.md" >}}) | [Boost.Beast]({{< relref "posts/boost/Boost.Beast_HTTP与WebSocket.md" >}}) | **Boost.JSON** | [Boost.MySQL]({{< relref "posts/boost/Boost.MySQL_异步数据库访问.md" >}})

## 前置知识

- [课程 1: Boost.System]({{< relref "posts/boost/Boost.System_错误处理基石.md" >}})（`error_code` 用于安全解析）
...
> **设计选择**：不抛异常，解析失败返回 `nullptr`。调用者可以通过 `val.is_null()` 判断。这与 [课程 1]({{< relref "posts/boost/Boost.System_错误处理基石.md" >}}) 中的 error_code 模式一致——可恢复错误用错误码/返回值，不用异常。
...
> 框架设计角度的深入讲解，请参见 [Hical 框架系列]({{< relref "posts/hical/01-pmr-memory-pool-design.md" >}})。
