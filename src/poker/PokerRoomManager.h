...
public:
  ...
  // BUG FIX: 加入 players 字段，HTTP join 响应即可携带完整玩家列表，
  // 无需等待 WS room_update 异步到来（避免好友端初始显示空槽位）
  struct JoinResult {
    bool ok;
    std::string token;
    std::string errMsg;
...
private:
  ...
  Json::Value _buildGameStateMsg(const PokerRoom &room,
                                 const std::string &forOpenId);
  void _broadcastGameState(PokerRoom &room);
  void _startCountdownTimer(PokerRoom &room);
  void _cleanupOldRooms();

  // 令牌校验
  bool _checkToken(const PokerRoom &room, const std::string &openId,
                   const std::string &token);
  // 找玩家
  PokerConnPlayer *_findPlayer(PokerRoom &room, const std::string &openId);
  const PokerConnPlayer *_findPlayer(const PokerRoom &room,
